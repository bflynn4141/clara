...
import { getTransactionsForPosition, calculateEarnings, loadYieldHistory, } from "../storage/yield-history.js";
import { getYieldOpportunities, createYieldPlan, createWithdrawPlan, executeYieldDeposit, executeYieldWithdraw, getYieldPositions, getTokenBalance, getBalances, encodeApproveCalldata, resolveToken, sendTransaction, waitForTransaction, checkGasForTransaction, executeSwap, getBridgeQuote, executeBridge, waitForBridge, } from "../para/client.js";
 ...
 */
async function ensureGasForOperation(chain, operation) {
...
Supported: USDC, USDT, DAI on Base and Arbitrum (Aave v3, Compound v3)`,
        ...
        try {
            ...
            if (action === "withdraw") {
                if (!asset) {
                    return { content: [{ type: "text", text: `❌ Please specify which asset to withdraw (e.g., asset="USDC")` }] };
                }
                if (!chain) {
                    return { content: [{ type: "text", text: `❌ Please specify which chain to withdraw from (e.g., chain="base")` }] };
                }
                return await handleWithdraw(asset, amount || "all", chain);
            }
            if (!asset) {
                return { content: [{ type: "text", text: `❌ Please specify which asset (e.g., asset="USDC")\n\nOr use action="positions" to see your current deposits.` }] };
            }
            if (!amount) {
            ...
            if (needsBridge && autoBridge && bridgeFromChain && bridgeToChain) {
                try {
                    bridgeQuote = await getBridgeQuote(asset, asset, amount, bridgeFromChain, bridgeToChain, { slippage: 0.5 });
                }
                catch (error) {
            ...
            if (action === "plan") {
                ...
                return { content: [{ type: "text", text: planDisplay + bridgeNote + "\n\nTo deposit, run again with action=\"deposit\"" }] };
            }
            // Execute the deposit (with bridge if needed)
            const outputLines = [planDisplay, ""];
            // Determine total steps for progress indicator
            const steps = getDepositSteps(needsBridge, needsBridge && bridgeQuote?.needsApproval || false, plan.needsApproval);
            let currentStep = 0;
                ...
                try {
                    ...
                    if (bridgeQuote.needsApproval && bridgeQuote.approvalAddress) {
                        ...
                        if (resolved) {
                            ...
                            const approvalResult = await sendTransaction(resolved.address, "0", bridgeFromChain, undefined, approvalData);
                    ...
                    if (shouldWaitForBridge) {
                        ...
                        outputLines.push("");
                        try {
                            const finalStatus = await waitForBridge(bridgeResult.txHash, bridgeFromChain, bridgeToChain, {
                    ...
                    return { content: [{ type: "text", text: `❌ Bridge failed: ${error instanceof Error ? error.message : "Unknown error"}` }] };
                }
            }
            if (plan.needsApproval) {
                ...
                if (!plan.approvalAddress) {
                    return { content: [{ type: "text", text: `❌ Approval address not available.` }] };
                }
                ...
                const approvalResult = await sendTransaction(plan.assetAddress, "0", plan.chain, undefined, approvalData);
                ...
                const approvalReceipt = await waitForTransaction(approvalResult.txHash, plan.chain, { pollIntervalMs: 3000, timeoutMs: 60000 });
            ...
            outputLines.push("");
            outputLines.push(`✅ Deposit complete!`, "", `Transaction: ${result.txHash}`, `Status: ${result.status}`, "", `Your ${plan.asset} is now earning ${plan.apy.toFixed(2)}% APY on ${plan.protocol}.`);
            return { content: [{ type: "text", text: outputLines.join("\n") }] };
        }
        catch (error) {
 ...
 */
function getDepositSteps(needsBridge, bridgeNeedsApproval, depositNeedsApproval) {
    const steps = [];
    if (needsBridge) {
 ...
 */
function calculateFeeBreakeven(depositAmount, apy, totalFeesUsd) {
 ...
 */
function formatPlanWithBridge(plan, bridgeQuote, fromChain, toChain) {
 ...
 */
async function handleApproval(plan, prefixLines = []) {
    if (!plan.approvalAddress) {
        return { content: [{ type: "text", text: `❌ Approval address not available.` }] };
    }
    const lines = [
    ...
    try {
        ...
        return { content: [{ type: "text", text: `❌ Approval failed: ${error instanceof Error ? error.message : "Unknown error"}` }] };
    }
}
...
async function showPositions(chains) {
    try {
        ...
        const lines = [`📊 Your Yield Positions`, ""];
        let totalCurrentValue = 0;
        let totalEarned = 0;
        for (const pos of positions) {
            const currentBalance = parseFloat(pos.deposited);
            totalCurrentValue += currentBalance;
            let earnings = { netDeposited: currentBalance, earnedYield: 0, earnedYieldPercent: 0, periodDays: 0, effectiveApy: null };
            if (session?.address) {
        ...
        return { content: [{ type: "text", text: `❌ Error fetching positions: ${error instanceof Error ? error.message : "Unknown error"}` }] };
    }
}
/**
...
async function handleWithdraw(asset, amount, chain) {
    try {
        ...
        if (!plan) {
            return { content: [{ type: "text", text: `❌ Cannot create withdrawal plan for ${asset.toUpperCase()} on ${capitalizeFirst(chain)}. Use action="positions" to see your current deposits.` }] };
        }
        ...
        return { content: [{ type: "text", text: `❌ Withdrawal failed: ${error instanceof Error ? error.message : "Unknown error"}` }] };
    }
}
/**
 * Show transaction history
 */
async function showTransactionHistory(assetFilter) {
    try {
        const session = await getSession();
        if (!session?.address) {
        ...
        if (history.transactions.length === 0) {
            return { content: [{ type: "text", text: `📜 Yield Transaction History\n\nNo transactions recorded yet.` }] };
        }
        ...
        return { content: [{ type: "text", text: `❌ Error fetching history: ${error instanceof Error ? error.message : "Unknown error"}` }] };
    }
}
