...
import { getTokenBalance, resolveToken, getTokenMetadata, POPULAR_TOKENS, } from "../para/client.js";
const SUPPORTED_CHAINS = ["ethereum", "base", "arbitrum", "optimism", "polygon"];
export function registerTokenBalanceTool(server) {
    server.registerTool("wallet_token_balance", {
...
Supported tokens by symbol: USDC, USDT, DAI, WETH, WBTC`,
        ...
        try {
            ...
            if (!token) {
                return await showAllTokenBalances(chain, session.address);
            }
            // Resolve token
            let tokenAddress;
            let tokenSymbol;
            const knownToken = resolveToken(token, chain);
            if (knownToken) {
            ...
            // Get balance
            const balanceInfo = await getTokenBalance(tokenAddress, chain, session.address);
            return {
                content: [{
                        type: "text",
                        text: `💰 Token Balance\n\n` +
 ...
 */
async function showAllTokenBalances(chain, ownerAddress) {
    const lines = [`💰 Token Balances on ${chain.charAt(0).toUpperCase() + chain.slice(1)}`, ""];
