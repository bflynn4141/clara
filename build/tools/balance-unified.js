...
import { getPortfolioFast, formatUsd, formatChange, reverseResolveEns, getTokenBalance, getBalances, resolveToken, getSolanaAssets, POPULAR_TOKENS, } from "../para/client.js";
import { isZerionAvailable, getPortfolioZerion, } from "../para/zerion.js";
const EVM_CHAINS = ["ethereum", "base", "arbitrum", "optimism", "polygon"];
const ALL_CHAINS = ["ethereum", "base", "arbitrum", "optimism", "polygon", "solana"];
const STABLECOINS_TO_CHECK = ["USDC", "USDT", "DAI"];
export function registerUnifiedBalanceTool(server) {
...
Supported tokens: ETH, MATIC, SOL, USDC, USDT, DAI, WETH, WBTC (or any contract address)`,
        ...
        try {
            ...
            if (!chain && !token) {
                ...
                return await showChainBalances(chain, session.address, session.solanaAddress, showEmpty);
            }
            else if (!chain && token) {
                ...
                return await showSpecificBalance(chain, token, session.address, session.solanaAddress);
            }
        }
        catch (error) {
 ...
 */
async function showFullPortfolio(address, solanaAddress, showEmpty) {
    ...
    if (items.length === 0) {
        ...
        for (const item of items) {
            ...
            lines.push(`│  ${chainName.padEnd(12)} │ ${balanceStr.padEnd(14)} │ ${priceStr.padEnd(10)} │ ${valueStr.padEnd(10)} │`);
        }
    }
    lines.push(`├─────────────────────────────────────────────────────────────┤`);
 ...
 */
async function showChainBalances(chain, address, solanaAddress, showEmpty) {
    const chainDisplay = chain.charAt(0).toUpperCase() + chain.slice(1);
    const lines = [`💰 ${chainDisplay} Balances`, ""];
    if (chain === "solana") {
        if (!solanaAddress) {
            return {
                content: [{
        ...
        if (isZerionAvailable()) {
            try {
                ...
                const chainPositions = portfolio.positions.filter(pos => pos.chain === chain && pos.positionType === "wallet");
 ...
 */
async function showTokenAcrossChains(token, address, solanaAddress, showEmpty) {
    const tokenUpper = token.toUpperCase();
    const isNative = ["ETH", "MATIC", "SOL"].includes(tokenUpper);
    const lines = [`💰 ${tokenUpper} Balances Across Chains`, ""];
    let totalValue = 0;
    let foundAny = false;
    ...
    if (isZerionAvailable() && tokenUpper !== "SOL") {
        try {
            ...
            const tokenPositions = portfolio.positions.filter(pos => pos.symbol.toUpperCase() === tokenUpper && pos.positionType === "wallet");
    ...
    if (!foundAny && !isZerionAvailable()) {
        const checks = EVM_CHAINS.map(async (chain) => {
            try {
 ...
 * Specific token on specific chain
 */
async function showSpecificBalance(chain, token, address, solanaAddress) {
    const chainDisplay = chain.charAt(0).toUpperCase() + chain.slice(1);
    const tokenUpper = token.toUpperCase();
    // Handle Solana
    if (chain === "solana") {
...
//# sourceMappingURL=balance-unified.js.map