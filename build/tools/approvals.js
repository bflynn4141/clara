...
import { getApprovalHistory, getAllowance, formatApproval, encodeApproveCalldata, getTokenMetadata, sendTransaction, } from "../para/client.js";
const SUPPORTED_CHAINS = ["ethereum", "base", "arbitrum", "optimism", "polygon"];
export function registerApprovalsTool(server) {
    server.registerTool("wallet_approvals", {
...
⚠️ Warning: Revoking approvals costs gas. Only revoke if you're concerned about security.`,
        ...
        try {
            ...
            if (action === "list") {
                return await listApprovals(chain, session.address);
            }
            if (action === "check") {
                if (!token || !spender) {
                    return {
                ...
                return await checkApproval(chain, token, spender);
            }
            if (action === "revoke") {
                if (!token || !spender) {
                    return {
                ...
                return await revokeApproval(chain, token, spender);
            }
            return {
                content: [{
                        type: "text",
 ...
 */
async function listApprovals(chain, _address) {
 ...
 */
async function checkApproval(chain, tokenInput, spenderAddress) {
 ...
 */
async function revokeApproval(chain, tokenInput, spenderAddress) {
    // Resolve token address
    let tokenAddress;
    let tokenSymbol;
    let tokenDecimals;
    ...
    try {
        ...
        const result = await sendTransaction(tokenAddress, // to: the token contract
        "0", // amount: 0 ETH value
        chain, undefined, // tokenAddress: not needed
