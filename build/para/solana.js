 ...
 */
export function isHeliusAvailable() {
 ...
 * - NFT holdings
 */
export async function getSolanaAssets(address) {
 ...
 */
async function getSolanaAssetsHelius(address) {
    ...
    const totalValueUsd = (solValue || 0) +
 ...
 */
async function getSolanaAssetsBasic(address) {
 ...
 */
export async function getSplTokenBalance(mintAddress, walletAddress) {
    ...
    // Check if it's SOL
    if (mintAddress.toLowerCase() === "sol" ||
        mintAddress === "So11111111111111111111111111111111111111112") {
        return portfolio.nativeBalance;
    }
    // Find in tokens
    return portfolio.tokens.find((t) => t.mintAddress.toLowerCase() === mintAddress.toLowerCase()) || null;
}
/**
 * Get Solana transaction history
 * Uses Helius enhanced transaction API for rich data
 */
export async function getSolanaTransactions(address, limit = 20) {
    const walletAddress = address || (await getSolanaAddress());
    if (!walletAddress) {
        throw new Error("No Solana wallet configured");
    }
    ...
    const response = await fetch(`https://api.helius.xyz/v0/addresses/${walletAddress}/transactions?api-key=${HELIUS_API_KEY}&limit=${limit}`);
    if (!response.ok) {
 ...
 */
function parseHeliusTransactions(data, walletAddress) {
    return data.map((tx) => {
        const isIncoming = tx.nativeTransfers?.some((t) => t.toUserAccount === walletAddress);
        return {
...
function formatTxType(type, isIncoming) {
    const typeMap = {
 ...
 */
export function resolveTokenMint(symbolOrMint) {
    ...
    throw new Error(`Unknown token symbol: ${symbolOrMint}. Please provide the mint address directly.`);
}
