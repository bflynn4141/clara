 ...
 */
export function recipientError(input: string): string {
 ...
 */
export function parseAndValidateAmount(amount: string): { valid: true; value: number } | { valid: false; error: string } {
 ...
 */
export function amountError(amount: string, context?: string): string {
  const result = parseAndValidateAmount(amount);
  if (result.valid) return ""; // shouldn't happen, but safe
 ...
 */
export function isValidTokenInput(token: string): boolean {
 ...
 */
export function tokenError(token: string): string {
 ...
 */
export function gasGuidance(chain: string): string {
