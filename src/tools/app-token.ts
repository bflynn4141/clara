...
const SUPPORTED_CHAINS = ["base"] as const;
 ...
 */
export function registerAppTokenTools(server: McpServer) {
  // ═══════════════════════════════════════════════════════════════════════
      ...
      try {
        ...
        if (session?.authenticated && session.address) {
          ...
          lines.push(
            ``,
            `═══ Your Position ═══`,
            `Balance: ${ethers.formatEther(balance)} ${symbol} (${ownership}%)`,
            `Claimable dividends: ${ethers.formatEther(dividendInfo[0])} ETH`,
            `Previously claimed: ${ethers.formatEther(dividendInfo[1])} ETH`,
          );
